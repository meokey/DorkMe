...
#---------------------------MAIN FUNC---------------------------
    ...
    def mainfunc(self):  
        ...
        dork = filex.readlines() #asign openned lines to dork list# encoding: utf-8
...
#---------------------------SHOW AND SAVE RESULTS---------------------------
            if self.__response: #only execute this loop if google have response 
                ...
                report_file.write("[*] CATEGORY: " + self.__category + "\n") #print in screen the category
                for j in self.__response: # for each link in response....
                    ...
                    report_file.write(resultone) #then write to file
                    line_len  = len(report_file.readline()) #read lines
                    report_file.seek(line_len) #set pointer at the end of line
                report_file.close() #close report file
            else:
                print("[*] no results\n")
